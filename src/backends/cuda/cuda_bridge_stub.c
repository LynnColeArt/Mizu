...
#define MIZU_CUDA_CONTEXT_PAGE_LAYOUT_OFFSET INT32_C(416)
#define MIZU_CUDA_CONTEXT_PAGE_LAYOUT_STRIDE INT32_C(24)
#define MIZU_CUDA_CONTEXT_PAGE_CONTROL_OFFSET INT32_C(512)
...
#define MIZU_CUDA_CONTEXT_TENSOR_ELEMENT_BYTES INT32_C(4)
#define MIZU_CUDA_CONTEXT_CHECKSUM_OFFSET UINT32_C(2166136261)
#define MIZU_CUDA_CONTEXT_CHECKSUM_PRIME UINT32_C(16777619)
#define MIZU_CUDA_PAGE_FLAG_RESIDENT INT32_C(1)
#define MIZU_CUDA_PAGE_FLAG_FULL INT32_C(2)
#define MIZU_CUDA_PAGE_FLAG_DECODE_OWNED INT32_C(4)
#define MIZU_CUDA_PAGE_FLAG_RECYCLED INT32_C(8)
...
static uint64_t mix_pack_page_words(uint64_t seed,
                                    const int64_t *pack_entry_page_hashes,
                                    const int32_t *pack_entry_page_word_counts,
                                    const int32_t *pack_entry_page_words,
                                    int32_t usage_index) {
    int32_t page_word_count;
    int32_t page_base;
    int32_t word_index;

    if (pack_entry_page_hashes == NULL || pack_entry_page_word_counts == NULL ||
        pack_entry_page_words == NULL) {
        return seed;
    }

    page_word_count = pack_entry_page_word_counts[usage_index];
    if (page_word_count < 0) page_word_count = 0;
    if (page_word_count > MIZU_CUDA_PACK_PAGE_WORDS) page_word_count = MIZU_CUDA_PACK_PAGE_WORDS;
    if (page_word_count <= 0) return seed;

    page_base = usage_index * MIZU_CUDA_PACK_PAGE_WORDS;
    seed = mix_u64(seed ^
        (uint64_t)pack_entry_page_hashes[usage_index] ^
        ((uint64_t)(uint32_t)(usage_index + 1) << 20));
    for (word_index = 0; word_index < page_word_count; ++word_index) {
        seed = mix_u64(seed ^
...
static uint64_t read_pack_index_bits(const int32_t *pack_entry_pack_indices,
                                     int32_t usage_index) {
    if (pack_entry_pack_indices == NULL) return UINT64_C(0);
    if (pack_entry_pack_indices[usage_index] <= 0) return UINT64_C(0);
    return (uint64_t)(uint32_t)pack_entry_pack_indices[usage_index];
}

static uint64_t mix_pack_entry_descriptor(uint64_t seed,
                                          ...
                                          int32_t usage_index) {
    uint64_t pack_index_bits;

    pack_index_bits = read_pack_index_bits(pack_entry_pack_indices, usage_index);
    seed = mix_u64(seed ^ (pack_index_bits << 20) ^
        ((uint64_t)(uint32_t)(usage_index + 1) << 40));

    if (pack_entry_offsets == NULL || pack_entry_bytes == NULL ||
        pack_role_codes == NULL || pack_layout_codes == NULL) {
        return seed;
    }

    return mix_u64(seed ^
        (uint64_t)pack_entry_offsets[usage_index] ^
        (uint64_t)pack_entry_bytes[usage_index] ^
        ((uint64_t)(uint32_t)pack_role_codes[usage_index] << 32) ^
        ((uint64_t)(uint32_t)pack_layout_codes[usage_index] << 48) ^
        (pack_index_bits << 8));
}

static uint64_t build_pack_execution_seed(uint64_t base_seed,
    ...
    seed = mix_u64(base_seed ^ pack_usage_hash);
    seed = mix_u64(seed ^ pack_usage_bytes ^
        (first_pack_offset << 1) ^ (last_pack_offset << 3) ^ (last_pack_bytes << 5) ^
        ((uint64_t)(uint32_t)pack_usage_count << 48));

    for (usage_index = 0; usage_index < pack_usage_count &&
         usage_index < MIZU_CUDA_CONTEXT_PACK_DISPATCH_COUNT; ++usage_index) {
        uint64_t entry_seed;
        const int has_tile_bytes = (pack_entry_tile_byte_counts != NULL &&
            pack_entry_tile_byte_counts[usage_index] > 0);
        const int has_page_words = (pack_entry_page_word_counts != NULL &&
...
static uint64_t digest_window_state(const uint64_t page_words[MIZU_CUDA_CONTEXT_PAGE_COUNT],
    ...
    for (index = 0; index < MIZU_CUDA_CONTEXT_PAGE_COUNT; ++index) {
        digest = mix_u64(digest ^ page_words[index] ^ page_lane_digests[index] ^
            (uint64_t)(uint32_t)page_key_rows[index] ^
            ((uint64_t)(uint32_t)page_key_lane_counts[index] << 16) ^
            ((uint64_t)(uint32_t)page_value_rows[index] << 24) ^
            ((uint64_t)(uint32_t)page_value_lane_counts[index] << 32) ^
            ((uint64_t)(uint32_t)page_head_blocks[index] << 40) ^
            ((uint64_t)(uint32_t)page_generations[index] << 48) ^
            mix_u64((uint64_t)(uint32_t)page_owner_kinds[index] ^
                ((uint64_t)(uint32_t)page_usable_capacities[index] << 8) ^
                ((uint64_t)(uint32_t)page_committed_tokens[index] << 16) ^
                ((uint64_t)(uint32_t)page_free_slots[index] << 24) ^
                ((uint64_t)(uint32_t)page_epochs[index] << 32) ^
                ((uint64_t)(uint32_t)page_recycle_epochs[index] << 40) ^
                ((uint64_t)(uint32_t)page_logical_ids[index] << 48) ^
                ((uint64_t)(uint32_t)page_flags[index] << 56)) ^
            (uint64_t)index);
    }
...
static void populate_page_tensor_layout(uint64_t page_word,
                                        int32_t generation,
                                        int32_t *key_rows,
    ...
    if (key_rows == NULL || key_lane_counts == NULL || value_rows == NULL || value_lane_counts == NULL ||
        head_block == NULL || page_generation == NULL) {
        return;
    }

    page_anchor = unpack_summary_leading_u16(page_word);
    page_fill = unpack_summary_auxiliary_u16(page_word);
    if (page_fill <= 0) {
        *key_rows = 0;
        *key_lane_counts = 0;
        *value_rows = 0;
...
static int32_t pack_page_control_flags(int32_t owner_kind,
...
static void populate_page_control(uint64_t page_word,
                                  int32_t owner_kind,
                                  int32_t page_epoch,
                                  int32_t recycle_epoch,
                                  ...
                                  int32_t *page_flags) {
    int32_t page_fill;
    int32_t resolved_owner_kind;

    if (page_owner_kind == NULL || usable_capacity == NULL || committed_tokens == NULL ||
        free_slots == NULL || resolved_page_epoch == NULL || resolved_recycle_epoch == NULL ||
        resolved_logical_page_id == NULL || page_flags == NULL) {
        return;
    }

    page_fill = unpack_summary_auxiliary_u16(page_word);
    resolved_owner_kind = owner_kind > 0 ? owner_kind : unpack_summary_control_b_u16(page_word);
    if (page_fill <= 0) {
        *page_owner_kind = 0;
        *usable_capacity = 0;
        *committed_tokens = 0;
...
static int32_t compute_page_storage_offset(int32_t payload_base_offset,
                                           int32_t page_index,
...
static int32_t compute_page_committed_bytes(int32_t committed_rows, int32_t lane_count) {
    if (committed_rows <= 0 || lane_count <= 0) return 0;
    return committed_rows * lane_count * MIZU_CUDA_CONTEXT_TENSOR_ELEMENT_BYTES;
}

static int32_t compute_page_capacity_bytes(int32_t usable_capacity, int32_t lane_count) {
    if (usable_capacity <= 0 || lane_count <= 0) return 0;
    return usable_capacity * lane_count * MIZU_CUDA_CONTEXT_TENSOR_ELEMENT_BYTES;
}

static int32_t compute_page_row_stride_bytes(int32_t lane_count) {
...
static void build_prefill_window_block(uint64_t seed,
    ...
    for (index = 0; index < MIZU_CUDA_CONTEXT_PAGE_COUNT && remaining_tokens > 0; ++index) {
        ...
        populate_page_tensor_layout(page_words[index], 0, &page_key_rows[index], &page_key_lane_counts[index],
            &page_value_rows[index], &page_value_lane_counts[index], &page_head_blocks[index],
            &page_generations[index]);
        populate_page_control(page_words[index], page_kind, index + 1, 0, index + 1, &page_owner_kinds[index],
            &page_usable_capacities[index], &page_committed_tokens[index], &page_free_slots[index],
            &page_epochs[index], &page_recycle_epochs[index], &page_logical_ids[index], &page_flags[index]);
    ...
    if (state_image_digest != NULL) {
        *state_image_digest = digest_window_state(page_words, recent_tokens, key_slot_lanes, value_slot_lanes,
            ...
            page_head_blocks, page_generations, page_owner_kinds, page_usable_capacities,
            page_committed_tokens, page_free_slots, page_epochs, page_recycle_epochs, page_logical_ids,
            page_flags, seed ^ (uint64_t)kv_token_count);
    }
}
...
static void write_context_pack_dispatch_block(uint8_t *bytes,
                                              int32_t stored_count,
                                              const int64_t *pack_entry_offsets,
                                              const int64_t *pack_entry_bytes,
                                              const int32_t *pack_role_codes,
    ...
    for (index = 0; index < MIZU_CUDA_CONTEXT_PACK_DISPATCH_COUNT; ++index) {
        const int32_t entry_offset = MIZU_CUDA_CONTEXT_PACK_DISPATCH_OFFSET +
            (index * MIZU_CUDA_CONTEXT_PACK_DISPATCH_STRIDE);
        const int entry_is_live = (index < pack_usage_count && pack_entry_offsets != NULL && pack_entry_bytes != NULL &&
            pack_role_codes != NULL && pack_layout_codes != NULL);
        write_context_u64(bytes, stored_count, entry_offset,
            entry_is_live ? (uint64_t)pack_entry_offsets[index] : UINT64_C(0));
        write_context_u64(bytes, stored_count, entry_offset + 8,
            entry_is_live ? (uint64_t)pack_entry_bytes[index] : UINT64_C(0));
        write_context_i32(bytes, stored_count, entry_offset + 16,
            entry_is_live ? pack_role_codes[index] : 0);
        write_context_i32(bytes, stored_count, entry_offset + 20,
            entry_is_live ? pack_layout_codes[index] : 0);
    }
}
...
static void build_decode_window_block(const uint64_t current_page_words[MIZU_CUDA_CONTEXT_PAGE_COUNT],
    ...
    int32_t valid_page_count;
    int32_t recent_token_count;
    int32_t max_page_epoch;
    int32_t max_logical_page_id;
    int32_t index;
    int emitted;
    ...
    if (emitted > 0) {
        ...
        if (!appended_to_existing_page) {
            if (valid_page_count < MIZU_CUDA_CONTEXT_PAGE_COUNT) {
                ...
                int32_t recycled_reuse_epoch;

                recycled_reuse_epoch = next_page_recycle_epochs[MIZU_CUDA_CONTEXT_PAGE_COUNT - 1] > 0 ?
...
static void fill_prefill_context_bytes(uint64_t seed,
    ...
    *context_byte_count = 0;
    ...
    build_prefill_window_block(seed, token_values, token_count, consumed_token_count, page_words, recent_tokens,
                               ...
                               page_generations, page_owner_kinds, page_usable_capacities,
                               page_committed_tokens, page_free_slots, page_epochs, page_recycle_epochs,
                               page_logical_ids, page_flags,
...
void mizu_cuda_bridge_prefill(int64_t payload_hash,
    ...
    *consumed_token_count = token_count > 0 ? token_count : 0;
    ...
    workspace_seed = build_pack_execution_seed(
        (uint64_t)artifact_hash ^ mix_u64((uint64_t)payload_hash),
        (uint64_t)pack_usage_hash,
        (uint64_t)pack_usage_bytes,
        (uint64_t)first_pack_offset,
        (uint64_t)last_pack_offset,
    ...
    workspace_seed = mix_u64(workspace_seed ^ tensor_seed ^
        (uint64_t)token_count ^
        ((uint64_t)modal_byte_count << 20) ^
        ((uint64_t)(uint32_t)staged_modal_count << 44));
...
void mizu_cuda_bridge_decode(int64_t payload_hash,
    ...
    seed = build_pack_execution_seed(
        (uint64_t)artifact_hash ^ mix_u64((uint64_t)payload_hash),
        (uint64_t)pack_usage_hash,
        (uint64_t)pack_usage_bytes,
        (uint64_t)first_pack_offset,
    ...
    seed = mix_u64(seed ^ current_window_meta ^ current_state_image_digest ^
        (uint64_t)kv_before ^ ((uint64_t)token_budget << 24));
    ...
    *stop_reason = MIZU_STOP_REASON_NONE;
    ...
    build_decode_window_block(current_page_words, current_recent_tokens, current_key_slot_lanes,
                              ...
                              (int32_t)unpack_state_decode_steps(next_state_lanes[2]),
    ...
    fill_decode_context_bytes(seed ^ (uint64_t)(uint32_t)(*token_value), (uint64_t)artifact_hash,
                              ...
                              (uint64_t)last_pack_bytes, pack_usage_count, pack_entry_offsets, pack_entry_bytes,
                              pack_role_codes, pack_layout_codes, next_state_lanes, summary_word, next_page_words, next_recent_tokens,
                              ...
                              next_page_owner_kinds, next_page_usable_capacities, next_page_committed_tokens,
                              next_page_free_slots, next_page_epochs, next_page_recycle_epochs,
                              next_page_logical_ids, next_page_flags, next_window_meta, next_state_image_digest,
