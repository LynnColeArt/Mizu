...
static int32_t is_apple_silicon_available(void) {
    int arm64_available = 0;
    size_t value_size = sizeof(arm64_available);

    if (sysctlbyname("hw.optional.arm64", &arm64_available, &value_size, NULL, 0) != 0) {
        return 0;
    }
    return arm64_available != 0 ? 1 : 0;
}
...
void mizu_apple_bridge_get_device_info(int32_t *metal_available,
    ...
    @autoreleasepool {
        ...
        if (metal_available != NULL) *metal_available = metal_device != nil ? 1 : 0;
        if (ane_available != NULL) *ane_available = is_apple_silicon_available();
        if (metal_device != nil) {
            copy_name([[metal_device name] UTF8String], device_name, device_name_capacity);
        } else if (ane_available != NULL && *ane_available != 0) {
            copy_name("apple_silicon", device_name, device_name_capacity);
        } else {
            copy_name("apple_unavailable", device_name, device_name_capacity);
        }
