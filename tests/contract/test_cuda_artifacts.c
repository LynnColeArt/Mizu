...
int main(void) {
    ...
    command_status = system("find /tmp/mizu_cuda_artifacts/artifacts/cuda/cuda/weights -name '*.packtiles' -exec grep -q \"kind=cuda_weight_pack_tile_cache_v3\" {} +");
    if (!expect_true("cuda weight pack tile cache should store the expected format marker", command_status == 0)) return 1;
    command_status = system("find /tmp/mizu_cuda_artifacts/artifacts/cuda/cuda/weights -name '*.packtiles' -exec grep -q \"pack_payload=.*.packpayload\" {} +");
    if (!expect_true("cuda weight pack tile cache should reference a dedicated pack payload", command_status == 0)) return 1;
    command_status = system("find /tmp/mizu_cuda_artifacts/artifacts/cuda/cuda/weights -name '*.packtiles' -exec grep -q \"pack_buffer=.*.packbuffer\" {} +");
    if (!expect_true("cuda weight pack tile cache should reference a dedicated pack buffer", command_status == 0)) return 1;
    ...
    command_status = system("find /tmp/mizu_cuda_artifacts/artifacts/cuda/cuda/weights -name '*.packtiles' -exec grep -q \"pack1_page_data_offset=\" {} +");
    if (!expect_true("cuda weight pack tile cache should record staged page data offsets", command_status == 0)) return 1;
    command_status = system("find /tmp/mizu_cuda_artifacts/artifacts/cuda/cuda/weights -name '*.packtiles' -exec grep -q \"pack1_tile_data_offset=\" {} +");
    if (!expect_true("cuda weight pack tile cache should record staged tile data offsets", command_status == 0)) return 1;
    ...
    command_status = system("find /tmp/mizu_cuda_artifacts/artifacts/cuda/cuda/plans -name '*.spancache' -exec grep -q \"entry1_pack=1\" {} +");
    if (!expect_true("cuda span-cache sidecar should retain compact pack indices", command_status == 0)) return 1;
