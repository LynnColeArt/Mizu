...
- current milestone: CUDA weight-pack `.packtiles` caches now derive their
  staged page/tile records from pack materialization metadata and pack
  identity, not just sampled importer preview bytes
...
- those same CUDA `.packtiles` payloads now derive their page/tile records from
  ...
  span reconstruction
- those same CUDA weight-pack caches now also materialize a dedicated
  `.packpayload` sibling carrying the staged page/tile bytes themselves, and
  the CUDA executor now hydrates that payload directly instead of depending on
  embedded `.packtiles` previews
